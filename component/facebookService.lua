		...
		local function facebookListener(event)
			...
			if ("session" == event.type) then
				...
				elseif (command == 5) then -- posting photo
					...
					local attachment = {
						name = "Enless words. Play on " .. getDevice ..  " device!",
				...
				elseif (command == 10) then -- share score
					...
					facebook.request( "me/feed", "POST", attachment ) -- posting phot
				-- elseif (command == 20) then --upload score to facebook (highscore system)
				-- 	local attachment = {
				-- 		score = 
				-- 	}
